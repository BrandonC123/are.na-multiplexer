...
function Window({
    ...
    const [loadingStatus, setLoadingStatus] =
        useState<LoadingStatusState>("inactive");
    ...
    const [draggingBlock, setDraggingBlock] =
        useState<ArenaChannelContents | null>();

    const blockPageSize = 50;
    const totalPages = useMemo(
        () => Math.ceil(channel.length / blockPageSize),
        [channel]
    );
    ...
    const fetchBlocks = useCallback(async () => {
        ...
        const results = await arena
            ...
            .contents({ page, per: blockPageSize });

        try {
            dispatchBlocks({
                type: "prepend",
                blocks: results.contents as ArenaChannelContents[],
            });
        } catch (error) {
    ...
    const canDelete = useMemo(
        () => channel.user_id === sessionData?.user.id,
        [channel, sessionData?.user]
    );
    ...
    useDndMonitor({
        onDragOver(event) {
            ...
            const { block } = active.data.current as {
                block: DraggingBlockData["block"];
            };
        ...
        onDragEnd(event) {
            ...
            const { window } = active.data.current as {
                window: DraggingBlockData["window"];
            };
    ...
    const disconnectBlock = useCallback(
        async (block: ArenaChannelContents) => {
            ...
            dispatchBlocks({
                type: "update",
                block: { ...block, ...{ connection_id: undefined } },
            });
            ...
            try {
                ...
                dispatchBlocks({
                    type: "update",
                    block: { ...block, ...{ connection_id: targetId } },
                });
            }
        },
    ...
    return (
        <MosaicWindow
            ...
            path={path}
            toolbarControls={
                <WindowToolbar data={data} setTumblr={setTumblr} />
            }
        >
            <div
            ...
            >
                ...
                {!blocks.length &&
                    loadingStatus !== "active" &&
                    tumblr.length === 0 && <BlankSlate />}

                {tumblr.length > 0 && <TumblrPosts tumblr={tumblr} />}
